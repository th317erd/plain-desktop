...
function mountOverlay(root: HTMLElement, image: ImageData, options: CaptureOverlayMountOptions): CaptureOverlayMount {
...
export async function bootstrapScreenCapture(): Promise<void> {
  ...
  const overlaySession = createCaptureOverlaySession({
    ...
    mount: (image, options) => mountOverlay(root, image, options),
  })
